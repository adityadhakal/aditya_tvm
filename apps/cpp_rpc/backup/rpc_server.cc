...
class RPCServer {
 ...
 private:
  ...
  void AcceptConnection(TrackerClient* tracker,
    ...
    static uint accept_conn_counter = 0;
    accept_conn_counter++;
    LOG(INFO)<<"Before while(true) in AcceptConnection. Accept conn counter "<<accept_conn_counter;
    ...
    while (true) {
      ...
      LOG(INFO)<<"The address we received from WaitConnectionAndUpdateKey "<<addr->AsString();
